        ...
        <div className={`flex flex-col min-h-screen ${isDarkMode ? 'bg-black text-white' : 'bg-white text-black'}`}>
            ...
            <main className="flex-grow container mx-auto px-4 py-8">
                ...
                <div className="grid md:grid-cols-3 gap-8">
                    <div className="md:col-span-2 space-y-4">
                        {items.map((item) => (
                            <div key={`${item.id}-${item.size}-${item.color}`} className="flex gap-4 p-4 border rounded-lg">
                                <Image src={item.image} alt={item.title} width={500} height={500} className="w-24 h-24 object-cover rounded" />
                    ...
                    <div className={`${isDarkMode ? 'bg-[#111]' : 'bg-gray-50'} p-6 rounded-lg h-fit`}>
                        ...
                        <Link
                            href="/checkout"
                            className={`block w-full py-3 text-center rounded-lg transition-colors ${isDarkMode
